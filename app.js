...
function persistData() {
  try {
    ...
    alert("Browser storage limit reached! Try using online Image URLs instead of large local photo uploads.");
  }
}
...
function exportData() {
  ...
  a.href = url;
  a.download = `bloom_reading_journal_backup_${new Date().toISOString().slice(0,10)}.json`;
  a.click();
...
function importData(e) {
  ...
  reader.onload = function(event) {
    try {
      ...
      if (imported.books || imported.wishlist) {
        books = imported.books || [];
        wishlist = imported.wishlist || [];
        readingGoal = Number(imported.goal) || 25;
        persistData();
        render();
        renderWishlist();
        alert("✅ All your books and wishlists were imported successfully!");
      } else {
...
function render() {
  ...
  if (filteredBooks.length === 0) {
    container.innerHTML = `<div style="grid-column: 1/-1; text-align:center; padding: 3rem; color: var(--text-muted); font-style: italic;">No books found. Click "+ Add New Book" or use "Import Data" if you backed it up.</div>`;
  }
...
document.getElementById('apiSearch')?.addEventListener('input', (e) => {
  ...
  debounceTimer = setTimeout(() => executeSearch(q, 'main', 'searchResults'), 500);
});
...
document.getElementById('wishApiSearch')?.addEventListener('input', (e) => {
  ...
  debounceTimer = setTimeout(() => executeSearch(q, 'wish', 'wishSearchResults'), 500);
});
...
async function executeSearch(query, type, resultsContainerId) {
  ...
  try {
    const q1 = encodeURIComponent(`intitle:${query}`);
    let res = await fetch(`https://www.googleapis.com/books/v1/volumes?q=${q1}&maxResults=6`);
    let data = await res.json();

    if (!data.items || data.items.length === 0) {
      const q2 = encodeURIComponent(query);
      res = await fetch(`https://www.googleapis.com/books/v1/volumes?q=${q2}&maxResults=6`);
      data = await res.json();
    }
  ...
  if (itemsFound.length === 0) {
    try {
      ...
      console.warn('Open Library fallback error:', err);
    }
  }

  resultsBox.innerHTML = '';
  if (itemsFound.length === 0) {
    resultsBox.innerHTML = '<div class="search-status">No matching books found online. You can enter details manually below!</div>';
    return;
  }
  ...
  itemsFound.forEach(book => {
    ...
    div.innerHTML = `
      ...
      <div class="search-text">
        ...
        <span style="color:var(--text-muted); font-size:0.75rem;">${escapeHtml(book.author)} ${book.pages ? `• ${book.pages} pages` : ''}</span>
      </div>
    `;
